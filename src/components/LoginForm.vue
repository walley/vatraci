<template>
  <div class="login-form white" color="primary">
    <v-alert
      ...
      :value="loginFailed"
      dismissible
      >{{ $t("global.login.failed") }}</v-alert
    >
    <div
      class="login-form__wrapper d-flex flex-column align-center justify-center"
    >
      <v-progress-circular
        v-if="loginWait"
        :size="100"
      ...
      <template v-else>
        ...
        >
          ...
          <v-btn
            ...
            type="submit"
            >{{ $t("global.login.submit") }}</v-btn
          >
        </v-form>
        <div>{{ this.fadVersion }}</div>
      </template>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapState("auth", ["loginWait", "loginFailed"]),
    ...mapState(["locales"]),
    loginRegex() {
      return auth.loginRegex
        ? auth.loginRegex
        : auth.loginWithEmail
        ? this.emailRegex
        : this.alphanumericRegex;
    },
    loginRules() {
      return [
        (v) => !!v || this.$t("global.login.loginRequired"),
        (v) =>
          this.emailRegex.test(v) || this.$t("global.login.incorrectLogin"),
      ];
    },
    passwordRegex() {
    ...
    fadVersion() {
      return main.fadVersion;
    },
  },
  methods: {
    ...mapMutations(["setLocale"]),
    ...mapActions("auth", ["login"]),
    ...mapActions("fad", ["actionPostLogon"]),
    changeLocale(locale) {
