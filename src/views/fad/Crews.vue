  ...
  <v-container fluid>
    <v-card flat>
      ...
      >
        ...
        <template v-slot:[`item.showInOrder`]="{ item }">
          <v-icon>{{ item.showInOrder ? "mdi-check" : "mdi-close" }}</v-icon>
        </template>
    ...
    <v-dialog v-model="dialog.show" max-width="600" persistent>
      <v-card>
        <v-card-title class="headline">
          {{ dialog.action === "create" ? "Nový výjezd" : "Výjezd" }}
        </v-card-title>
        <v-form v-model="dialog.formValid">
          <v-card-text>
            ...
            ></v-text-field>
            <v-checkbox
              v-model="dialog.form.showInOrder"
              :label="'Zobrazit v rozkazu'"
              hide-details
            ></v-checkbox>
          </v-card-text>
          <v-card-actions>
...
export default {
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        { text: "Zobrazit v rozkazu", value: "showInOrder" },
  ...
  methods: {
    ...
    loadItem(id) {
      return this.$http.get(`crud/fad/crews/${id}`);
    },
    createItem(params) {
      return this.$http.post("crud/fad/crews", params);
    },
    updateItem(id, params) {
      return this.$http.put(`crud/fad/crews/${id}`, params);
    },
    destroyItem(id) {
      return this.$http.delete(`crud/fad/crews/${id}`);
    },
    ...
    edit(item) {
      this.loadItem(item.id).then(
        ...
        (error) => {
          this.showMessage(error.statusText || "Chyba při načítání výjezdu", "error");
        }
      );
    },
    store() {
      this.createItem({
        ...
        showInOrder: this.dialog.form.showInOrder ? 1 : 0,
      }).then(
        (response) => {
          if (response.body.status === 0) {
            this.showMessage("Výjezd vytvořen", "success");
            this.dialog.show = false;
            this.loadItems();
          } else {
            this.showMessage(response.body.msg || "Chyba při vytváření výjezdu", "error");
          }
        },
        (error) => {
          this.showMessage(error.statusText || "Chyba při vytváření výjezdu", "error");
        }
      );
    },
    update() {
      this.updateItem(this.dialog.id, {
        ...
        (response) => {
          if (response.body.status === 0) {
            this.showMessage("Výjezd upraven", "success");
            this.dialog.show = false;
            this.loadItems();
          } else {
            this.showMessage(response.body.msg || "Chyba při úpravě výjezdu", "error");
          }
        },
        (error) => {
          this.showMessage(error.statusText || "Chyba při úpravě výjezdu", "error");
        }
      );
    },
    suspend(item) {
      this.updateItem(item.id, { active: 0 }).then(
        (response) => {
          if (response.body.status === 0) {
            this.showMessage("Výjezd suspendován", "success");
            this.loadItems();
          } else {
    ...
    restore(item) {
      this.updateItem(item.id, { active: 1 }).then(
        (response) => {
          if (response.body.status === 0) {
            this.showMessage("Výjezd obnoven", "success");
            this.loadItems();
          } else {
    ...
    destroy(item) {
      if (!confirm("Opravdu chcete trvale odstranit tento výjezd?")) return;
      this.destroyItem(item.id).then(
        () => {
          this.showMessage("Výjezd trvale odstraněn", "success");
          this.loadItems();
        },
        (error) => {
          this.showMessage(error.statusText || "Chyba při odstraňování výjezdu", "error");
        }
      );
    },
    suspendSelected() {
      ...
      Promise.all(promises).then(() => {
        this.showMessage("Vybrané výjezdy suspendovány", "success");
        this.selected = [];
        this.loadItems();
      });
    },
    restoreSelected() {
      ...
      Promise.all(promises).then(() => {
        this.showMessage("Vybrané výjezdy obnoveny", "success");
        this.selected = [];
        this.loadItems();
      });
    },
    destroySelected() {
      if (!confirm("Opravdu chcete trvale odstranit vybrané výjezdy?")) return;
      const promises = this.selected.map((item) => this.destroyItem(item.id));
      Promise.all(promises).then(() => {
        this.showMessage("Vybrané výjezdy trvale odstraněny", "success");
        this.selected = [];
        this.loadItems();
      });
    },
