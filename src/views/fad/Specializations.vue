    ...
    <v-dialog v-model="dialog.show" max-width="600" persistent>
      <v-card>
        <v-card-title class="headline">
          {{ dialog.action === "create" ? "Nová odbornost" : "Odbornost" }}
        </v-card-title>
...
export default {
  ...
  methods: {
    ...
    loadItem(id) {
      return this.$http.get(`crud/fad/specializations/${id}`);
    },
    createItem(params) {
      return this.$http.post("crud/fad/specializations", params);
    },
    updateItem(id, params) {
      return this.$http.put(`crud/fad/specializations/${id}`, params);
    },
    ...
    edit(item) {
      this.loadItem(item.id).then(
        ...
        (error) => {
          this.showMessage(error.statusText || "Chyba při načítání odbornosti", "error");
        }
      );
    },
    store() {
      this.createItem({
        ...
        (response) => {
          if (response.body.status === 0) {
            this.showMessage("Odbornost vytvořena", "success");
            this.dialog.show = false;
            this.loadItems();
          } else {
            this.showMessage(response.body.msg || "Chyba při vytváření odbornosti", "error");
          }
        },
        (error) => {
          this.showMessage(error.statusText || "Chyba při vytváření odbornosti", "error");
        }
      );
    },
    update() {
      this.updateItem(this.dialog.id, {
        ...
        (response) => {
          if (response.body.status === 0) {
            this.showMessage("Odbornost upravena", "success");
            this.dialog.show = false;
            this.loadItems();
          } else {
            this.showMessage(response.body.msg || "Chyba při úpravě odbornosti", "error");
          }
        },
        (error) => {
          this.showMessage(error.statusText || "Chyba při úpravě odbornosti", "error");
        }
      );
    },
    suspend(item) {
      this.updateItem(item.id, { active: 0 }).then(
        (response) => {
          if (response.body.status === 0) {
            this.showMessage("Odbornost suspendována", "success");
            this.loadItems();
          } else {
    ...
    restore(item) {
      this.updateItem(item.id, { active: 1 }).then(
        (response) => {
          if (response.body.status === 0) {
            this.showMessage("Odbornost obnovena", "success");
            this.loadItems();
          } else {
    ...
    suspendSelected() {
      ...
      Promise.all(promises).then(() => {
        this.showMessage("Vybrané odbornosti suspendovány", "success");
        this.selected = [];
        this.loadItems();
      });
    },
    restoreSelected() {
      ...
      Promise.all(promises).then(() => {
        this.showMessage("Vybrané odbornosti obnoveny", "success");
        this.selected = [];
        this.loadItems();
      });
    },
