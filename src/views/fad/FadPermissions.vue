    ...
    <v-dialog v-model="dialog.show" max-width="600" persistent>
      <v-card>
        <v-card-title class="headline">
          {{ dialog.action === "create" ? "Nové FAD oprávnění" : "FAD oprávnění" }}
        </v-card-title>
...
export default {
  ...
  methods: {
    ...
    loadItem(id) {
      return this.$http.get(`crud/fad/fad-permissions/${id}`);
    },
    createItem(params) {
      return this.$http.post("crud/fad/fad-permissions", params);
    },
    updateItem(id, params) {
      return this.$http.put(`crud/fad/fad-permissions/${id}`, params);
    },
