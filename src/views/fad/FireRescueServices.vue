    ...
    <v-dialog v-model="dialog.show" max-width="600" persistent>
      <v-card>
        <v-card-title class="headline">
          {{ dialog.action === "create" ? "Nový hasičský záchranný sbor" : "Hasičský záchranný sbor" }}
        </v-card-title>
  ...
  methods: {
    ...
    loadItems() {
      ...
      this.$http
        ...
        .then(
          ...
          (error) => {
            this.loading = false;
            this.showMessage(error.statusText || "Chyba při načítání sborů", "error");
          }
        );
    },
    loadItem(id) {
      return this.$http.get(`crud/fad/fire-rescue-services/${id}`);
    },
    createItem(params) {
      return this.$http.post("crud/fad/fire-rescue-services", params);
    },
    updateItem(id, params) {
      return this.$http.put(`crud/fad/fire-rescue-services/${id}`, params);
    },
    destroyItem(id) {
      return this.$http.delete(`crud/fad/fire-rescue-services/${id}`);
    },
    ...
    edit(item) {
      this.loadItem(item.id).then(
        ...
        (error) => {
          this.showMessage(error.statusText || "Chyba při načítání sboru", "error");
        }
      );
    },
    store() {
      this.createItem({ name: this.dialog.form.name }).then(
        (response) => {
          if (response.body.status === 0) {
            this.showMessage("Sbor vytvořen", "success");
            this.dialog.show = false;
            this.loadItems();
          } else {
            this.showMessage(response.body.msg || "Chyba při vytváření sboru", "error");
          }
        },
        (error) => {
          this.showMessage(error.statusText || "Chyba při vytváření sboru", "error");
        }
      );
    },
    update() {
      this.updateItem(this.dialog.id, { name: this.dialog.form.name }).then(
        (response) => {
          if (response.body.status === 0) {
            this.showMessage("Sbor upraven", "success");
            this.dialog.show = false;
            this.loadItems();
          } else {
            this.showMessage(response.body.msg || "Chyba při úpravě sboru", "error");
          }
        },
        (error) => {
          this.showMessage(error.statusText || "Chyba při úpravě sboru", "error");
        }
      );
    },
    suspend(item) {
      this.updateItem(item.id, { active: 0 }).then(
        (response) => {
          if (response.body.status === 0) {
            this.showMessage("Sbor suspendován", "success");
            this.loadItems();
          } else {
    ...
    restore(item) {
      this.updateItem(item.id, { active: 1 }).then(
        (response) => {
          if (response.body.status === 0) {
            this.showMessage("Sbor obnoven", "success");
            this.loadItems();
          } else {
    ...
    destroy(item) {
      if (!confirm("Opravdu chcete trvale odstranit tento sbor?")) return;
      this.destroyItem(item.id).then(
        () => {
          this.showMessage("Sbor trvale odstraněn", "success");
          this.loadItems();
        },
        (error) => {
          this.showMessage(error.statusText || "Chyba při odstraňování sboru", "error");
        }
      );
    },
    suspendSelected() {
      ...
      Promise.all(promises).then(() => {
        this.showMessage("Vybrané sbory suspendovány", "success");
        this.selected = [];
        this.loadItems();
      });
    },
    restoreSelected() {
      ...
      Promise.all(promises).then(() => {
        this.showMessage("Vybrané sbory obnoveny", "success");
        this.selected = [];
        this.loadItems();
      });
    },
    destroySelected() {
      if (!confirm("Opravdu chcete trvale odstranit vybrané sbory?")) return;
      const promises = this.selected.map((item) => this.destroyItem(item.id));
      Promise.all(promises).then(() => {
        this.showMessage("Vybrané sbory trvale odstraněny", "success");
        this.selected = [];
        this.loadItems();
      });
    },
