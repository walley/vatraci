    ...
    <v-dialog v-model="dialog.show" max-width="600" persistent>
      <v-card>
        <v-card-title class="headline">
          {{ dialog.action === "create" ? "Nová požární stanice" : "Požární stanice" }}
        </v-card-title>
        <v-form v-model="dialog.formValid">
          <v-card-text>
            ...
            ></v-text-field>
            <v-autocomplete
              v-model="dialog.form.TerritorialDistrictId"
              :items="territorialDistricts"
              :loading="territorialDistrictsLoading"
              item-text="name"
              item-value="id"
              :label="'Územní odbor'"
              hide-details
...
export default {
  ...
  data() {
    return {
      ...
      territorialDistrictsLoading: false,
      loading: true,
      search: "",
      selectedStatuses: [1],
      selected: [],
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        { text: "Územní odbor", value: "territorialDistrict" },
    ...
    mappedItems() {
      return this.items.map((r) => ({
        ...
        name: r.name,
        active: r.active,
        territorialDistrict: r.TerritorialDistrict ? r.TerritorialDistrict.name : "-",
        _raw: r,
      }));
    },
    filteredItems() {
  ...
  methods: {
    ...
    loadTerritorialDistricts() {
      this.territorialDistrictsLoading = true;
      this.$http
        .get("crud/fad/territorial-districts")
        .then(
          (response) => {
            this.territorialDistricts = (response.body || []).filter((t) => t.active == 1);
            this.territorialDistrictsLoading = false;
          },
          (error) => {
            this.territorialDistrictsLoading = false;
    ...
    loadItem(id) {
      return this.$http.get(`crud/fad/fire-stations/${id}`);
    },
    createItem(params) {
      return this.$http.post("crud/fad/fire-stations", params);
    },
    updateItem(id, params) {
      return this.$http.put(`crud/fad/fire-stations/${id}`, params);
    },
    ...
    create() {
      this.dialog = {
        ...
        action: "create",
        id: null,
        form: { name: "", TerritorialDistrictId: null },
        formValid: false,
      };
    },
    edit(item) {
      this.loadItem(item.id).then(
        (response) => {
          ...
          this.dialog = {
            ...
            form: {
              name: r.name,
              TerritorialDistrictId: r.TerritorialDistrictId,
            },
            formValid: false,
          };
        },
        (error) => {
          this.showMessage(error.statusText || "Chyba při načítání stanice", "error");
        }
      );
    },
    store() {
      this.createItem({
        ...
        TerritorialDistrictId: this.dialog.form.TerritorialDistrictId,
      }).then(
        (response) => {
          if (response.body.status === 0) {
            this.showMessage("Stanice vytvořena", "success");
            this.dialog.show = false;
            this.loadItems();
          } else {
            this.showMessage(response.body.msg || "Chyba při vytváření stanice", "error");
          }
        },
        (error) => {
          this.showMessage(error.statusText || "Chyba při vytváření stanice", "error");
        }
      );
    },
    update() {
      this.updateItem(this.dialog.id, {
        ...
        (response) => {
          if (response.body.status === 0) {
            this.showMessage("Stanice upravena", "success");
            this.dialog.show = false;
            this.loadItems();
          } else {
            this.showMessage(response.body.msg || "Chyba při úpravě stanice", "error");
          }
        },
        (error) => {
          this.showMessage(error.statusText || "Chyba při úpravě stanice", "error");
        }
      );
    },
    suspend(item) {
      this.updateItem(item.id, { active: 0 }).then(
        (response) => {
          if (response.body.status === 0) {
            this.showMessage("Stanice suspendována", "success");
            this.loadItems();
          } else {
    ...
    restore(item) {
      this.updateItem(item.id, { active: 1 }).then(
        (response) => {
          if (response.body.status === 0) {
            this.showMessage("Stanice obnovena", "success");
            this.loadItems();
          } else {
    ...
    suspendSelected() {
      ...
      Promise.all(promises).then(() => {
        this.showMessage("Vybrané stanice suspendovány", "success");
        this.selected = [];
        this.loadItems();
      });
    },
    restoreSelected() {
      ...
      Promise.all(promises).then(() => {
        this.showMessage("Vybrané stanice obnoveny", "success");
        this.selected = [];
        this.loadItems();
      });
    },
