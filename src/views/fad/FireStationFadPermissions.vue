    ...
    <v-dialog v-model="dialog.show" max-width="600" persistent>
      <v-card>
        <v-card-title class="headline">
          {{ dialog.action === "create" ? "Nové FAD oprávnění - požární stanice" : "FAD oprávnění - požární stanice" }}
        </v-card-title>
...
<script>
export default {
  name: "FireStationFadPermissions",
  data() {
    return {
      items: [],
      permissions: [],
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        { text: "Id", value: "id" },
        { text: "FAD Oprávnění", value: "permission" },
  ...
  methods: {
    ...
    loadItems() {
      this.loading = true;
      this.$http
        .get("crud/fad/fire-station-fad-permissions")
        .then(
          (response) => {
          ...
          (error) => {
            this.loading = false;
            this.showMessage(error.statusText || "Chyba při načítání oprávnění stanic", "error");
          }
        );
    },
    loadPermissions() {
    ...
    loadItem(id) {
      return this.$http.get(`crud/fad/fire-station-fad-permissions/${id}`);
    },
    createItem(params) {
      return this.$http.post("crud/fad/fire-station-fad-permissions", params);
    },
    updateItem(id, params) {
      return this.$http.put(`crud/fad/fire-station-fad-permissions/${id}`, params);
    },
    destroyItem(id) {
      return this.$http.delete(`crud/fad/fire-station-fad-permissions/${id}`);
    },
      ...
      this.createItem({
        ...
        (response) => {
          if (response.body.status === 0) {
            this.showMessage("Oprávnění stanice vytvořeno", "success");
    ...
    update() {
      this.updateItem(this.dialog.id, {
        ...
        (response) => {
          if (response.body.status === 0) {
            this.showMessage("Oprávnění stanice upraveno", "success");
            this.dialog.show = false;
            this.loadItems();
          } else {
