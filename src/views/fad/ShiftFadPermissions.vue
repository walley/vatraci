    ...
    <v-dialog v-model="dialog.show" max-width="600" persistent>
      <v-card>
        <v-card-title class="headline">
          {{ dialog.action === "create" ? "Nové FAD oprávnění - směna" : "FAD oprávnění - směna" }}
        </v-card-title>
...
export default {
  ...
  methods: {
    ...
    loadItems() {
      this.loading = true;
      this.$http
        .get("crud/fad/shift-fad-permissions")
        .then(
          (response) => {
          ...
          (error) => {
            this.loading = false;
            this.showMessage(error.statusText || "Chyba při načítání oprávnění směn", "error");
          }
        );
    },
    loadPermissions() {
    ...
    loadItem(id) {
      return this.$http.get(`crud/fad/shift-fad-permissions/${id}`);
    },
    createItem(params) {
      return this.$http.post("crud/fad/shift-fad-permissions", params);
    },
    updateItem(id, params) {
      return this.$http.put(`crud/fad/shift-fad-permissions/${id}`, params);
    },
    destroyItem(id) {
      return this.$http.delete(`crud/fad/shift-fad-permissions/${id}`);
    },
      ...
      this.createItem({
        ...
        (response) => {
          if (response.body.status === 0) {
            this.showMessage("Oprávnění směny vytvořeno", "success");
    ...
    update() {
      this.updateItem(this.dialog.id, {
        ...
        (response) => {
          if (response.body.status === 0) {
            this.showMessage("Oprávnění směny upraveno", "success");
            this.dialog.show = false;
            this.loadItems();
          } else {
