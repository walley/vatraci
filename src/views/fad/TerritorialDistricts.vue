    ...
    <v-dialog v-model="dialog.show" max-width="600" persistent>
      <v-card>
        <v-card-title class="headline">
          {{ dialog.action === "create" ? "Nový územní odbor" : "Územní odbor" }}
        </v-card-title>
        <v-form v-model="dialog.formValid">
          <v-card-text>
            ...
            ></v-text-field>
            <v-autocomplete
              v-model="dialog.form.FireRescueServiceId"
              :items="fireRescueServices"
              :loading="fireRescueServicesLoading"
              item-text="name"
              item-value="id"
              :label="'HZS'"
              hide-details
...
<script>
export default {
  name: "TerritorialDistricts",
  data() {
    return {
      items: [],
      fireRescueServices: [],
      fireRescueServicesLoading: false,
      loading: true,
      search: "",
      selectedStatuses: [1],
      selected: [],
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        { text: "HZS", value: "fireRescueService" },
    ...
    mappedItems() {
      return this.items.map((r) => ({
        ...
        name: r.name,
        active: r.active,
        fireRescueService: r.FireRescueService ? r.FireRescueService.name : "-",
        _raw: r,
      }));
    },
    filteredItems() {
  ...
  methods: {
    ...
    loadItems() {
      this.loading = true;
      this.$http
        .get("crud/fad/territorial-districts")
        .then(
          (response) => {
          ...
          (error) => {
            this.loading = false;
            this.showMessage(error.statusText || "Chyba při načítání územních odborů", "error");
          }
        );
    },
    loadFireRescueServices() {
      this.fireRescueServicesLoading = true;
      this.$http
        .get("crud/fad/fire-rescue-services")
        .then(
          (response) => {
            this.fireRescueServices = (response.body || []).filter((f) => f.active == 1);
            this.fireRescueServicesLoading = false;
          },
          (error) => {
            this.fireRescueServicesLoading = false;
            this.showMessage(error.statusText || "Chyba při načítání HZS", "error");
          }
        );
    },
    loadItem(id) {
      return this.$http.get(`crud/fad/territorial-districts/${id}`);
    },
    createItem(params) {
      return this.$http.post("crud/fad/territorial-districts", params);
    },
    updateItem(id, params) {
      return this.$http.put(`crud/fad/territorial-districts/${id}`, params);
    },
    ...
    create() {
      this.dialog = {
        ...
        action: "create",
        id: null,
        form: { name: "", FireRescueServiceId: null },
        formValid: false,
      };
    },
    edit(item) {
      this.loadItem(item.id).then(
        (response) => {
          ...
          this.dialog = {
            ...
            form: {
              name: r.name,
              FireRescueServiceId: r.FireRescueServiceId,
            },
            formValid: false,
          };
        },
        (error) => {
          this.showMessage(error.statusText || "Chyba při načítání územního odboru", "error");
        }
      );
    },
    store() {
      this.createItem({
        ...
        FireRescueServiceId: this.dialog.form.FireRescueServiceId,
      }).then(
        (response) => {
          if (response.body.status === 0) {
            this.showMessage("Územní odbor vytvořen", "success");
            this.dialog.show = false;
            this.loadItems();
          } else {
            this.showMessage(response.body.msg || "Chyba při vytváření územního odboru", "error");
          }
        },
        (error) => {
          this.showMessage(error.statusText || "Chyba při vytváření územního odboru", "error");
        }
      );
    },
    update() {
      this.updateItem(this.dialog.id, {
        ...
        (response) => {
          if (response.body.status === 0) {
            this.showMessage("Územní odbor upraven", "success");
            this.dialog.show = false;
            this.loadItems();
          } else {
            this.showMessage(response.body.msg || "Chyba při úpravě územního odboru", "error");
          }
        },
        (error) => {
          this.showMessage(error.statusText || "Chyba při úpravě územního odboru", "error");
        }
      );
    },
    suspend(item) {
      this.updateItem(item.id, { active: 0 }).then(
        (response) => {
          if (response.body.status === 0) {
            this.showMessage("Územní odbor suspendován", "success");
            this.loadItems();
          } else {
    ...
    restore(item) {
      this.updateItem(item.id, { active: 1 }).then(
        (response) => {
          if (response.body.status === 0) {
            this.showMessage("Územní odbor obnoven", "success");
            this.loadItems();
          } else {
    ...
    suspendSelected() {
      ...
      Promise.all(promises).then(() => {
        this.showMessage("Vybrané územní odbory suspendovány", "success");
        this.selected = [];
        this.loadItems();
      });
    },
    restoreSelected() {
      ...
      Promise.all(promises).then(() => {
        this.showMessage("Vybrané územní odbory obnoveny", "success");
        this.selected = [];
        this.loadItems();
      });
    },
